...
# Type2 arguments
parser.add_argument('--num_images_type2', type=int, default=20)
parser.add_argument('--backend', type=str, default='CYCLES')
parser.add_argument('--light_mode', type=str, default='uniform')
parser.add_argument('--camera_pose_type2', type=str, default='random')
...
parser.add_argument('--extract_material_type2', action='store_true', default=True)
parser.add_argument('--delete_material', action='store_true')
parser.add_argument('--uniform_light_direction', nargs='+', default=['0.09387503', '-0.63953443', '-0.7630093'])
...
# Globals for type2 / basic_lighting (overwrite module-level None)
UNIFORM_LIGHT_DIRECTION = [float(x) for x in args.uniform_light_direction]
...
def setup_nodes(output_path, capturing_material_alpha: bool = False, basic_lighting: bool = False):
    ...
    if capturing_material_alpha:
        ...
        # "Linear Rec.2020" exists in Blender 3.4+; on macOS/older builds use "Linear"
        try:
...
def render_scene(output_path, fast_mode: bool, extract_material: bool, basic_lighting: bool, use_workbench: bool = True):
    ...
    if use_workbench:
        # Re-render RGBA using workbench with texture mode (type1 style).
...
def save_rendering_dataset(
    ...
    """Type2 pipeline: full scene setup with lights, renders 00000..num_images-1, writes transforms_train.json and info.json."""
...
def render_script_type1():
    ...
    for uid in uid_paths:
        ...
        # Type2 pipeline: same output dir, 00000..num_images-1, transforms_train.json, info.json
        try:
            save_rendering_dataset(
                input_path=uid,
                output_path=cur_path,
                num_images=args.num_images_type2,
                backend=args.backend,
                light_mode=args.light_mode,
                camera_pose=args.camera_pose_type2,
                camera_dist_min=args.camera_dist_min,
                camera_dist_max=args.camera_dist_max,
                fast_mode=args.fast_mode,
                extract_material=args.extract_material_type2,
                delete_material=args.delete_material,
            )
        except Exception as e:
            print(f"render_script_type2 failed for {uid}: {e}")
            try:
                save_rendering_dataset(
                    input_path=uid,
                    output_path=cur_path,
                ...
                print(f"render_script_type2 fallback also failed: {e2}")
